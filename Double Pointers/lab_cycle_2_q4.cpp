...
//	}
	...
	friend bool validateRowsAndColoumns(const Matrix&,const Matrix&,int);
};
...
bool validateRowsAndColoumns(const Matrix&A,const Matrix&B,int choice){
	//for addition
	if (choice==1) {
...
//		matrix2.allocateMatrix();
		...
		do {
			...
			switch (option) {
				case 1:
					if (validateRowsAndColoumns(matrix1, matrix2, 1)) {
						Matrix Sum;
						Sum = matrix1+matrix2;
						Sum.displayMatrix();
					} else {
